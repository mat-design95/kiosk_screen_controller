...
import electron, { app as app$1, screen, BrowserWindow, ipcMain as ipcMain$1 } from "electron";
...
const StorageService = {
  ...
  removeDisplayConfig: (id2) => {
    const displays = store.get("displays");
    delete displays[id2];
    store.set("displays", displays);
  },
  updateActiveDisplays: (activeDisplays) => {
    store.set("activeDisplays", activeDisplays);
  }
};
const DisplayManagerService = {
  getSystemDisplays() {
    if (!app$1.isReady()) return [];
  ...
  identifyDisplays() {
    ...
    displays.forEach((d, i) => {
      ...
      const identifyWin = new BrowserWindow2({
        x: d.bounds.x,
        y: d.bounds.y,
      ...
      const content2 = `data:text/html;charset=UTF-8,
        <html>
...
const activeWindows = {};
const activeRefreshTimers = {};
const watchdogReconnections = {};
let globalNightModeActive = false;
const WindowManagerService = {
  startKioskWindow(displayId) {
    var _a;
    ...
    if (!((_a = tempStore.activeDisplays) == null ? void 0 : _a.includes(displayId))) {
    ...
    kioskWindow.webContents.on("did-fail-load", (_evt, errorCode, errorDescription) => {
      if (globalNightModeActive) return;
      ...
      if (config.reloadOnError) {
        if (!watchdogReconnections[displayId]) {
          ...
          }, (config.errorTimeoutSeconds || 30) * 1e3);
        }
      }
    });
    ...
    if (config.refreshIntervalMinutes && config.refreshIntervalMinutes > 0) {
      ...
      }, config.refreshIntervalMinutes * 60 * 1e3);
    }
  ...
  stopKioskWindow(displayId) {
    ...
    if (activeWindows[displayId]) {
      ...
      const tempStore = StorageService.getStore();
      if ((_a = tempStore.activeDisplays) == null ? void 0 : _a.includes(displayId)) {
        StorageService.updateActiveDisplays(tempStore.activeDisplays.filter((id2) => id2 !== displayId));
      }
    }
  },
  stopAllWindows() {
    Object.keys(activeWindows).forEach((id2) => {
      this.stopKioskWindow(id2);
    });
  },
  restoreSession() {
    const store2 = StorageService.getStore();
    if (store2.globalSettings.restoreLastSession && store2.activeDisplays) {
      store2.activeDisplays.forEach((id2) => this.startKioskWindow(id2));
    }
  },
  cleanupTimers(displayId) {
  ...
  setNightModeForAll(isNight) {
    globalNightModeActive = isNight;
    Object.keys(activeWindows).forEach((id2) => {
      this.navigateWindow(id2);
    });
  },
  navigateWindow(displayId) {
    const win2 = activeWindows[displayId];
    if (!win2) return;
    if (globalNightModeActive) {
      const store2 = StorageService.getStore();
      const logoParam = store2.nightModeSettings.logoAssetPath || "electron-vite.svg";
      const bgHex = store2.nightModeSettings.backgroundMode === "black" ? "000000" : "1e1e1e";
      const devUrl = process.env["VITE_DEV_SERVER_URL"];
      if (devUrl) {
        win2.loadURL(`${devUrl}/nightmode.html?logo=${logoParam}&bg=${bgHex}`);
      } else {
        const publicPath = process.env.VITE_PUBLIC || path.join(__dirname, "../public");
        win2.loadFile(path.join(publicPath, "nightmode.html"), { query: { logo: logoParam, bg: bgHex } });
      }
    } else {
      const config = StorageService.getStore().displays[displayId];
      if (config && config.url) {
        win2.loadURL(config.url).catch(() => {
        });
      }
    }
  }
...
function checkNightMode() {
  const store2 = StorageService.getStore();
  if (!store2 || !store2.nightModeSettings.enabled) {
    if (isCurrentlyNight) endNightMode();
    return;
  }
  const { startTime, endTime } = store2.nightModeSettings;
...
const SchedulerService = {
  start() {
    ...
    checkInterval = setInterval(checkNightMode, 6e4);
    checkNightMode();
  },
  stop() {
    if (checkInterval) {
...
app$1.whenReady().then(() => {
  ...
  ipcMain$1.handle("get-store", () => StorageService.getStore());
  ipcMain$1.handle("get-system-displays", () => DisplayManagerService.getSystemDisplays());
  ipcMain$1.on("update-display-config", (_, id2, config) => {
    StorageService.updateDisplayConfig(id2, config);
  });
  ipcMain$1.on("update-global-settings", (_, settings) => {
    StorageService.updateGlobalSettings(settings);
  });
  ipcMain$1.on("update-nightmode-settings", (_, settings) => {
    StorageService.updateNightModeSettings(settings);
  });
  ipcMain$1.on("start-kiosk", (_, id2) => {
    WindowManagerService.startKioskWindow(id2);
  });
  ipcMain$1.on("stop-kiosk", (_, id2) => {
    WindowManagerService.stopKioskWindow(id2);
  });
  ipcMain$1.on("identify-displays", () => {
    DisplayManagerService.identifyDisplays();
  });
  const globalConfig = StorageService.getStore().globalSettings;
  app$1.setLoginItemSettings({
    openAtLogin: globalConfig.launchAtStartup,
